...
let getData = async (query) => {
  let data = await fetch(
    `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=${query}&key=AIzaSyB-5ye-k0psvQ7VTKviINcRkdnZEcPGttk`
  );
...
let getDataOnLoad = async () => {
  let data = await fetch(
    `https://youtube.googleapis.com/youtube/v3/search?part=snippet&chart=mostPopular&regionCode=IN&maxResults=50&key=AIzaSyB-5ye-k0psvQ7VTKviINcRkdnZEcPGttk`
  );
  let val = await data.json();
  return val.items;
};
